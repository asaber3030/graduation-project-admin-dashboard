  ...
  return (
    <div className='space-y-4'>
      <AdminPageTitle title={pageTitle}>
        ...
        <LinkBtn href={adminRoutes.doctors.update(doctor.id)} variant='destructive'>
          Delete
        </LinkBtn>
      </AdminPageTitle>
      ...
      <section className='grid xl:grid-cols-3 grid-cols-1 gap-4'>
        <section className='col-span-2'>
          <div className='flex items-center justify-between mb-2'>
            ...
            <LinkBtn href={adminRoutes.doctors.doctorLoginHistory(doctor.id)}>View All</LinkBtn>
          </div>
          ...
          <section className='print:hidden'>
            ...
            <section className='grid grid-cols-4 gap-2'>
              ...
              <Link
                href={adminRoutes.doctors.doctorPrescriptions(doctor.id)}
                className='border flex gap-2 items-center rounded-md shadow-sm font-medium hover:border-primary transition-colors px-4 py-2'
              >
                <Book className='size-4' /> Prescriptions
              </Link>
              <Link
                href={adminRoutes.doctors.doctorLoginHistory(doctor.id)}
                className='border flex gap-2 items-center rounded-md shadow-sm font-medium hover:border-primary transition-colors px-4 py-2'
              >
        ...
        <section>
          ...
          <Link
            href={adminRoutes.hospitals.view(hospital.id)}
